...
export { toBnNumber as toBnDigits };
...
export function banglaCalendarParts(date, locale = "bn") {
  ...
  if (locale === "en") {
    return {
      weekday,
      dateLine: `${bangla.day} ${bangla.month.en} ${bangla.year} BS`,
    };
  }
  return {
    weekday,
    dateLine: `${toBnNumber(bangla.day)} ${bangla.month.bn} ${toBnNumber(bangla.year)} বঙ্গাব্দ`,
  };
}

export function formatBanglaCalendar(date, locale = "bn") {
  const parts = banglaCalendarParts(date, locale);
  if (!parts) return "—";
  return `${parts.weekday}, ${parts.dateLine}`;
}
